...
| 3 | Running header: automatic book title + page number, mirrored on facing pages; opening page hidden by default. Chapter headings and ornaments stay on-demand document content. | ✅ done — public `bookHeader` option controls title/page-number/first-page visibility; book title is escaped before the pagination library renders it; build passes and pages 2–4 were browser-verified with correct mirroring. Automatic “current chapter” text is deferred because it needs a reliable heading-to-page map after every reflow; page-number-based overrides would drift as the author types. |
